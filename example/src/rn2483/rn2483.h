...
/*!
    Toggles the reset pin (from HIGH -> LOW and LOW -> HIGH).

    The RN2483 module transmits it's firmware version upon being reset, so if the version is succesfully returned then it's assume that the reset was successful.

    @return RN2483_SUCCESS if version was successfully retrieved after toggling the RESET pin
    @return RN2483_ERRPANIC if version was not retrieved after toggling the RESET pin
*/
